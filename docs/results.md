...
Every number here comes from running the system. None is estimated. This is the
long form of the summary table in [the README](../README.md); each section is
the milestone that produced the numbers in it.

Hardware for the rate-dependent measurements: **AMD Ryzen 7 3700X** (8 cores,
16 threads), 31 GiB RAM, ext4 on NVMe, Go 1.27, Linux.
...
are in [columnar.md](columnar.md).
...
isolation; none of it is tested joined up. [deploy.md](deploy.md)
keeps the same list.
...
in [decisions.md](decisions.md). There were three candidates — block, drop, buffer — and the
